...
- **Наш выбор**
MemPalace + OpenRouter (бесплатная модель)
==Почему?==
...
![Диалог с удалением конкретного факта](sequence_diagrams/sequence_base.svg)
...
Участвуют два тестовых пользователя для проверки изоляции.
  
### 3.2. Что считаем успешным пилотом  
  
**Инженерный критерий:**  
- accuracy на LongMemEval ≥ ?,
- TTFT p95 < 1 секунды,
...
|-----------|------------|------------|
| **WebUI** | ? | Интерфейс пользователя (чат, выбор персоны, кнопка "забыть") |
...
| **Embeddings** | ? | Векторизация фактов при добавлении в память |
...
- **Rate limiting:** ограничение 10 сообщений в минуту на пользователя
...
| **Бесплатная модель OpenRouter недоступна** | средняя | высокое (агент не отвечает) | Перейти на другую бесплатную модель. |
| **TTFT > 1 секунды из-за поиска в памяти** | средняя | среднее | Уменьшить `top_k` до 3, кэшировать эмбеддинги, использовать асинхронный поиск. |

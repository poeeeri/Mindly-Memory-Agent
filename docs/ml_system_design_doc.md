...
В случае, когда пользователь желает, чтобы его собеседник забыл весь контекст, он запрашивает в чате удаление:
...
| **Memory framework** | MemPalace | Хранение, поиск, удаление фактов. Под капотом: Chroma (векторная БД) + PostgreSQL (метаданные) |
